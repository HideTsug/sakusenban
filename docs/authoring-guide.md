...
色は**意味にのみ**付与する: 青=人間の実行と押せる操作（塗り=操作 / 淡色=状態で区別）/ 橙=共同・注意 / 緑=着手可・OK・正本確認済み / 赤=期限切迫 / グレーアウト=完了。端末ローカルのチェックは青塗り、正本確認済み（`.ssot-done`）のチェックは緑塗りで描画される。レーン背景・番号バッジ・装飾への色付与は禁止。新しい色を足すのは新しい「意味」ができた時だけ。
